        ...
        if (!file_exists($_FILES["aadhaar"]["tmp_name"][$i])) {
            ...
            if (move_uploaded_file($_FILES["aadhaar"]["tmp_name"][$i], $target_file)) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    $sql = "SELECT `limit` FROM `international_package` WHERE `place_name` = '$place_name'";
                    $result = mysqli_query($connection,$sql);
                    // $new_limit = `international_package`.'limit';  
                     $sql =  "UPDATE `international_package` SET `limit`= `limit`- 1   WHERE  `place_name` = '$place_name'" ;
                     $result = mysqli_query($connection,$sql); 
